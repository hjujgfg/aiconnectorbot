...
def list_procedures(
    start_time_utc: str = None, end_time_utc: str = None, client_id: int = None
) -> str:
...
class GeminiEngine:
    def __init__(
        self, api_key: Optional[str] = None, model_id: str = "gemini-3-flash-preview"
    ):
...
3. Если кандидатов нет, спроси: "Не нашла такого клиента. Создать нового?".
4. Номер телефона и заметки — необязательны.
5. Если в сообщении пользователь указывает что-то в скобках, то добавь это в колонку `notes`.
